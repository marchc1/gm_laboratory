...
static void RewriteClientClasses(ClientClassesEditor& editor) {
	...
	if (health.Valid()) {
		...
		g_api->Log("example5", "rerouted %s.%s recv proxy\n", kTargetClass, kTargetProp);
	}
	else {
		g_api->Log("exampl5e", "could not find %s.%s\n", kTargetClass, kTargetProp);
	}
}
...
static void RewriteServerClasses(ServerClassesEditor& editor) {
	...
	if (origin.Valid()) {
		...
		g_api->Log("example5", "set CBaseEntity.m_vecOrigin to noscale (flags now 0x%X)\n", origin.Flags());
	}
}

MODULE_MAIN() {
	g_api->Log("example5", "Loaded Example Module #5: Modifying RecvProp/SendProps (abi %u)\n", api->abiVersion);
